...
**`quip-brain` (spike — AI SDK direction):** a code-first agent brain (Hono + Vercel AI SDK v6, `@ai-sdk/fireworks`) is being trialled as a replacement for the n8n agent node. It exposes `POST /quip` matching the bot contract (`{…event} → {reply, confirm, gif}`), defines tools as typed `tool({inputSchema, execute})` functions (unit-tested with the SDK mock model — the thing n8n couldn't do), and reuses the same persona + `redact` hard-gate. Runs as a standalone container on `n8n-net` (no host port; `FIREWORKS_API_KEY` in its chmod-600 env_file). The bot has a `QUIP_USE_BRAIN` toggle (default **off** — n8n stays the production brain). First tool ported: `get_container_status`; verified live (identity rundown + tool call, MOC excluded). Plan: port the rest (notes/reminders/propose_action), keep n8n for cron/scheduling, then cut over. Part of the broader direction — Discord as the universal frontend, with a Hono+AI-SDK brain, a Python discord.py bot for Domain Hunter (replacing dh-web), and LangGraph for DH's pipeline — assessed in the Quip repo.
